...
if ($conn->connect_error) die("DB Error"); $user_id = $_SESSION['user_id']; /*
Book user wants */ $receiver_book_id = intval($_GET['id']); $target =
$conn->query(" SELECT b.*, u.firstname, u.lastname FROM books b JOIN users u ON
b.user_id = u.id WHERE b.id = $receiver_book_id ")->fetch_assoc(); /* IMPORTANT:
receiver (book owner) */ $receiver_id = $target['user_id']; /* User's own books
*/ $myBooks = $conn->query(" SELECT * FROM books WHERE user_id = $user_id AND
exchanged = 0 "); ?>
...
<body>
    ...
    <div class="container">
        <div class="post">
            ...
            <p>
                <?= htmlspecialchars($target['language']) ?>
                |
                <?= htmlspecialchars($target['type']) ?>
            </p>
        </div>
    ...
    <div class="popup-bg" id="popup">
        <div class="popup-box">
            ...
            <button class="btn" style="background: #dc3545" onclick="closePopup()">
                Cancel
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        function sendRequest() {
            ...
            fetch("save_exchange_request.php", {
                ...
                body: `sender_book_id=${selectedBook.id}
